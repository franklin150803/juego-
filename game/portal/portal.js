export class Portal {
  constructor(scene, position = new THREE.Vector3(0,0,-10)) {
    ...
    this.distortion=[];
    for(let i=0;i<28;i++){const a=Math.random()*Math.PI*2,r=5.4+Math.random()*5.5;const p=new THREE.Mesh(new THREE.SphereGeometry(.035+Math.random()*.045,6,5),new THREE.MeshBasicMaterial({color:0xd18aff,transparent:true,opacity:.18,blending:THREE.AdditiveBlending,depthWrite:false}));p.position.set(Math.cos(a)*r,2+Math.random()*5.2,Math.sin(a)*r);p.userData={a,r,y:p.position.y,speed:.4+Math.random()*1.3,phase:Math.random()*6.28};this.group.add(p);this.distortion.push(p)}
    this.shock=[];
    ...
    this.scene=scene;scene.add(this.group);scene.userData.portalPressure=0;
  }
  update(t,pressure=0){
    pressure=THREE.MathUtils.clamp(pressure,0,1);this.scene.userData.portalPressure=pressure;
    const pulse=Math.sin(t*5),violent=Math.pow(pressure,1.35);
    this.ring.rotation.z+=.009+pressure*.03;this.ring.scale.setScalar(1+pulse*.025+violent*.12);this.core.material.opacity=.82+Math.abs(pulse)*.1+violent*.12;this.light.intensity=4.5+pulse*.8+pressure*6.5;this.light.distance=24+pressure*10;
    ...
    this.distortion.forEach((p,i)=>{const d=p.userData;d.a+=d.speed*.012*(1+pressure*7);const pull=pressure*(1.2+Math.sin(t*2+d.phase)*.3);const rr=d.r*(1-.52*pull);p.position.x=Math.cos(d.a)*rr;p.position.z=Math.sin(d.a)*rr;p.position.y=d.y+Math.sin(t*2+d.phase)*.35*(1+pressure*2);p.material.opacity=.12+pressure*.65;p.scale.setScalar(1+pressure*2.2)});
    this.shock.forEach((s,i)=>{const cycle=(t*(.65+pressure*2)+i*.8)%2.4;const active=cycle<1.25,q=active?cycle/1.25:0;s.scale.setScalar(1+q*(1.6+pressure*2.2));s.material.opacity=active?(1-q)*(.12+pressure*.32):0});
  }
