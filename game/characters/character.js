export class ElementCharacter {
  constructor({ element, name, color, accent, glow, emoji = '✦', isPlayer = false }) {
    this.element = element; this.name = name; this.color = color; this.accent = accent; this.glow = glow; this.emoji = emoji; this.isPlayer = isPlayer;
    this.group = new THREE.Group(); this.velocity = new THREE.Vector3(); this.health = 100; this.maxHealth = 100; this.grounded = true;
    this._build();
  }
  ...
  _build() {
    const body = new THREE.Mesh(new THREE.CapsuleGeometry(.34, .72, 5, 10), this._mat(0x101116, this.accent, .35)); body.position.y = .82; body.castShadow = true; this.group.add(body); this.body = body;
    ...
    const eye = new THREE.MeshBasicMaterial({ color: this.glow });
    [-.09,.09].forEach(x => { const e = new THREE.Mesh(new THREE.SphereGeometry(.035, 8, 8), eye); e.position.set(x,1.65,.245); this.group.add(e); });
