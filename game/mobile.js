...
(function(){
  ...
  const held=new Set();
  const press=k=>{if(held.has(k))return;held.add(k);window.dispatchEvent(new KeyboardEvent('keydown',{key:k,code:k===' ':'Space':`Key${k.toUpperCase()}`,bubbles:true}))};
  const release=k=>{if(!held.has(k))return;held.delete(k);window.dispatchEvent(new KeyboardEvent('keyup',{key:k,code:k===' ':'Space':`Key${k.toUpperCase()}`,bubbles:true}))};
  ...
  // Ataque = Space; habilidad = E; tomar = F. Ya no hay dos botones enviando E.
